...
Simplified - Works directly with Sales Invoice
"""
        ...
        try:
            ...
            if response.status_code != 200:
                ...
                try:
                    ...
                    error_msg = error_detail[:100]
                
                frappe.log_error(
                    f"Paystack Error {response.status_code}: {error_msg}",
                    "Paystack API"
                )
                return {
                    "success": False,
                    "message": _("Paystack error: {0}").format(response.status_code)
                }
            ...
            return {
                "success": False,
                "message": _("Failed to connect to Paystack. Please try again.")
            }
...
def initiate_payment(sales_invoice, payment_amount, email=None):
    ...
    try:
        ...
        return {
            "success": False,
            "message": _("Payment initialization failed")
        }
...
def verify_payment(sales_invoice=None, reference=None):
    ...
    try:
        ...
        frappe.log_error(f"Verify payment: {reference}", "Paystack")
        
        gateway = PaystackGateway()
        verification = gateway.verify_payment(reference)
        
        if not verification:
            frappe.log_error("Verify failed - no response", "Paystack")
            frappe.local.response["type"] = "redirect"
            frappe.local.response["location"] = "/student-portal/fees?error=verification_failed"
            return
        
        if verification.get("status") and verification["data"].get("status") == "success":
            payment_data = verification["data"]
            frappe.log_error(f"Payment success: {reference}", "Paystack")
            ...
            if existing_payment:
                frappe.log_error(f"Duplicate payment: {existing_payment}", "Paystack")
                # Redirect to fees page
                frappe.local.response["type"] = "redirect"
                frappe.local.response["location"] = "/student-portal/fees?success=true"
                return
            
            try:
                payment_entry = _create_payment_entry(sales_invoice, payment_data)
                frappe.log_error(f"Payment entry created: {payment_entry.name}", "Paystack")
                ...
                frappe.log_error(f"Create entry failed: {str(e)[:80]}", "Paystack")
                # Redirect to fees page with error
                frappe.local.response["type"] = "redirect"
                frappe.local.response["location"] = "/student-portal/fees?error=payment_processing_failed"
                return
        else:
            # Payment failed
            frappe.log_error(f"Payment failed: {reference}", "Paystack")
            frappe.local.response["type"] = "redirect"
            frappe.local.response["location"] = "/student-portal/fees?error=payment_failed"
            return
    
    except Exception as e:
        frappe.log_error(f"Verify error: {str(e)[:80]}", "Paystack")
        # Redirect to fees page with error
        frappe.local.response["type"] = "redirect"
        frappe.local.response["location"] = "/student-portal/fees?error=verification_error"
        return
