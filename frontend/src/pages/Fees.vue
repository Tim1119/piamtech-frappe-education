 ...
 * Calls the API to get the configured format from School Settings
 */
const getPrintFormatForProgram = async (program) => {
  try {
    // Call the API to get the correct format based on program
    const result = await apiCall(
      `/api/method/piamtech_frappe_education.school_portal_api.get_print_format_for_program_whitelist?program=${encodeURIComponent(program)}&format_type=report`
    )
    
    const format = result.message || "Standard"
 ...
 * Opens the report in browser print preview
 */
const printReport = async (report) => {
  try {
    ...
    // Open print preview using /printview endpoint
